...
Const SiteManageCode = "GSjdh2016"      '后台管理认证密码，请修改，这样即使有人知道了您的后台用户名和密码也不能登录后台
Const IsBusiness=False              

 
If DataBaseType=0 then
	'如果是ACCESS数据库，请认真修改好下面的数据库的文件名
	DBPath       = "/jindongheng/wap/KS_Data/XunWang.mdb"      'ACCESS数据库的文件名，请使用相对于网站根目录的的绝对路径
Else
	 '如果是SQL数据库，请认真修改好以下数据库选项
	 DataServer   = "localhost"                                  '数据库服务器IP
	 DataUser     = "gsjdh"                                       '访问数据库用户名
	 DataBaseName = "gsjdh"                                '数据库名称
	 DataBasePsw  = "GSjdh2016"                                   '访问数据库密码 
End if

'采集数据库路径
CollectDBPath="/jindongheng/wap/KS_Data/Collect/KS_Collect.Mdb"
...
Const RootDomain = "gsjindongheng.com"       rem 网站主域名根,如果有多个子域名,必须设置
'=============================================二级域名配置结束========================================================
